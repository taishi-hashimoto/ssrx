...
int main(int argc, char* argv[]) {
    ...
    bool test = false;
    bool amphist = false;
    bool no_pps = true;  // HackRF doesn't support PPS synchronization, so default to true and ignore the option.
    bool test_integrity = false;
    auto path_yaml = ssrx::config::expand_path("~/.ssrx/conf/ssrx.yaml");

    CLI::App app{"SSR receiver program using HackRF One."};
    ...
    app.add_flag("--test", test, "Test configuration and HackRF, then exit.");
    ...
    app.add_flag("-n,--no-pps", no_pps, "Start sampling without latching time with the next PPS (no effect for HackRF).");
    ...
    using sample_raw_t = std::complex<int8_t>;
    ...
    using sdr_device_t = ssrx::sdr::HackRF<pipeline_data_t>;
    ...
    try {
        ...
        std::cerr << "Error: failed to initialize HackRF receiver: " << e.what() << std::endl;
        return EXIT_FAILURE;
    }
